...
class DeltaEmbeddingLoRA(torch.nn.Module):
    def __init__(self, embed_dim=768, rank=4):
        super().__init__()
        self.down = torch.nn.Linear(embed_dim, rank, bias=False)
        self.up = torch.nn.Linear(rank, embed_dim, bias=False)

    def forward(self, x):
        return self.up(self.down(x))  # learn residual direction
...
def train_lora(image,
    ...
    if is_sdxl:
        model = StableDiffusionXLPipeline.from_pretrained(model_path).to(device)
    ...
    with torch.no_grad():
        if is_sdxl:
            text_embedding, _, _, _ = model.encode_prompt(prompt)
            text_embedding = text_embedding.repeat(lora_batch_size, 1, 1).half()
        else:
    ...
    image_transforms = transforms.Compose([
        transforms.Resize(512, interpolation=transforms.InterpolationMode.BILINEAR),
        transforms.RandomCrop(512),  # 隨機 crop 而不是 center crop
        transforms.RandomHorizontalFlip(p=0.5),   # 加入隨機左右翻轉
        transforms.ColorJitter(0.1, 0.1, 0.1, 0.05),  # 輕度亮度、對比、飽和度、色調變化
        transforms.ToTensor(),
        transforms.Normalize([0.5], [0.5]),
        transforms.RandomErasing(p=0.2, scale=(0.02, 0.1)),  # 模擬 Cutout 效果
    ])
    ...
    if use_gradio_progress:
        progress_bar = progress.tqdm(range(lora_step), desc="training LoRA")
    else:
        progress_bar = tqdm(range(lora_step), desc="training LoRA")
