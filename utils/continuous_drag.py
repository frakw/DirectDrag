...
def drag_stretch_multipoint_ratio_interp_differentiable(
    invert_code, handle_points, target_points, radius=30
):
    """
    Differentiable version of drag_stretch_multipoint_ratio_interp using grid_sample.
    Applies weighted interpolation based on geometric distance and flow from handle to target points.

    Args:
        invert_code (torch.Tensor): shape (B, C, H, W)
        handle_points (list): list of (y, x) coordinates in pixel space
        target_points (list): list of (y, x) coordinates in pixel space
        radius (float): falloff parameter for weighting influence of each handle

    Returns:
        torch.Tensor: warped latent code of shape (B, C, H, W)
    """
    B, C, H, W = invert_code.shape
    device = invert_code.device
    dtype = invert_code.dtype
    ...
    identity_grid = torch.stack((grid_x, grid_y), dim=-1).unsqueeze(0).repeat(B, 1, 1, 1)  # (B, H, W, 2)
    total_flow = torch.zeros_like(identity_grid)
    weights_sum = torch.zeros((1, H, W, 1), device=device, dtype=dtype)

    for h_pt, t_pt in zip(handle_points, target_points):
        # Convert pixel coords to normalized [-1, 1]
        h_y, h_x = h_pt
        t_y, t_x = t_pt
        h_norm = torch.tensor([(h_x / (W - 1)) * 2 - 1, (h_y / (H - 1)) * 2 - 1], device=device, dtype=dtype)
        t_norm = torch.tensor([(t_x / (W - 1)) * 2 - 1, (t_y / (H - 1)) * 2 - 1], device=device, dtype=dtype)
        delta = (t_norm - h_norm).view(1, 1, 1, 2)

        # Distance to handle point in pixel space
        y_idx = torch.linspace(0, H - 1, H, device=device, dtype=dtype)
        x_idx = torch.linspace(0, W - 1, W, device=device, dtype=dtype)
        y_grid, x_grid = torch.meshgrid(y_idx, x_idx, indexing='ij')
        dist = torch.sqrt((y_grid - h_y) ** 2 + (x_grid - h_x) ** 2)
        weight = torch.exp(-dist / radius).unsqueeze(0).unsqueeze(-1)  # (1, H, W, 1)

        total_flow += delta * weight
        weights_sum += weight

    flow_grid = identity_grid + (total_flow / weights_sum.clamp(min=1e-8))

    # Apply grid_sample warp
    warped_latent = F.grid_sample(invert_code, flow_grid, mode='bilinear', padding_mode='border', align_corners=True)
