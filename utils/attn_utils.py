...
def override_attn_proc_forward(attn, editor, place_in_unet):
    def forward(x, encoder_hidden_states=None, attention_mask=None, context=None, mask=None, iter_cur=0, save_kv=True):
        """
...
def override_lora_attn_proc_forward(attn, editor, place_in_unet):
    def forward(hidden_states, encoder_hidden_states=None, attention_mask=None, lora_scale=1.0, iter_cur=0, save_kv=True):
        ...
        query = attn.to_q(hidden_states) #fastdrag kv copy
        ...
        key, value = attn.to_k(encoder_hidden_states), attn.to_v(encoder_hidden_states) #fastdrag kv copy
        ...
        hidden_states = attn.to_out[0](hidden_states) #fastdrag kv copy
        # dropout
